...
parser.add_argument('--dataset_name', type=str, default='gaussian_mixture',
                    choices=['gaussian_mixture', 'crowd_motion_gaussian', 
...
parser.add_argument('--tensor_type',        type=str, default='double', choices=['float', 'double']) 
...
if args.dataset_name == 'gaussian_mixture':
    ...
    X_train, _, _, train_loader, val_loader, test_loader, target_dist = make_crowd_motion_gaussian_data(args.gaussian_multi_dim, args.num_train_data, \
...
# crowd motion
Q = construct_Q(args, device, space=space)
...
def compute_loss_gradcheck(mode='train'):
    # forward
    log_density, _, _, hist_norm, hist_ld_norm, OT_cost_norm, _ = flow.log_prob(batch)
    log_density = torch.mean(log_density)
    ...
    if args.NF_loss == 'KL_sampling':
        G_cost = -log_density
        if target_dist is not None:
            log_prob_1  = torch.mean(target_dist.log_prob(batch))
            KL_sampling = log_prob_1 - log_density
    elif args.NF_loss == 'KL_density':
        assert target_dist is not None
        log_prob_0   = torch.mean(distribution.log_prob(z_0))
    ...
    with torch.no_grad():
        for k,p in enumerate(model.parameters()):
            ...
            for j in range(size):
                ...
                loss_perturbed = compute_loss(NF_copy, x, args, mode='train')['loss']
...
for step in tbar:
    ...
    # Pytorch FD checking
    # gradcheck(compute_loss_gradcheck, )

    # manual FD checking
    G = [p.grad.data for p in flow.parameters()]
    ...
    G_FD_list = []
    for d in d_list:
        G_FD = grad_FD(flow, batch, loss, d=d)
    ...
    # print ("grad from FD: {}".format(G_FD_list))
    # print ("grad from AD: {}".format(G))
