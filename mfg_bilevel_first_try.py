...
def parse_arguments():
    ...
    # Bilevel MFG
    parser.add_argument('--bilevel_training_data_dir', type=str, 
    default='./results/crowd_motion_gaussian/NSF_CL_crowd_motion_2D_Jeff_OT_1e-1_F_2e-1_B=2048_lr=1e-3_N=50k_FD4Simp/train_traj_crowd_motion_gaussian.pt')                            
    parser.add_argument('--num_training_steps_inner',  type=int, default=3,
                        help='Number of total training steps in the inner loop (lower problem)')
    ...
    parser.add_argument('--step_decay_inner',          type=float, default=0,
                        help='Decay factor for the lower problem step size, range: [0, inf], 0 = no decay')
    ...
    parser.add_argument('--grad_norm_clip_value_NF',   type=float, default=5.,
                        help='Value by which to clip norm of gradients.')
    parser.add_argument('--grad_norm_clip_value_obs',  type=float, default=5.,
                        help='Value by which to clip norm of gradients.')
...
print('There are {} trainable parameters in the flow model.'.format(n_params))
...
print('There are {} trainable parameters in the obstacle model.'.format(utils.get_num_parameters(obstacle)))
...
def compute_l_grad_i_star(NF, x):
    # x: B x K x d
    B = x.shape[0]
    _, _, _, hist_gen, _ = NF._transform.inverse(x[:,0,:])
    hist_gen = partition_hist(hist_gen, args, mode='gen', LU_last=args.LU_last, partition_mode=args.OT_part)
    F = hist_gen.view(B,-1) # B x K*d
    x = x.view(B, -1)
    grad_l = F - x # B x K*d

    return grad_l

def compute_mfg_obj(NF_obstacle, x, z, args, mode='train'):
    # forward
    log_density, _, _, hist_norm, _, _, _ = NF_obstacle.NF.log_prob(x)
    log_density = torch.mean(log_density)

    # backward
    # z_K, ld_gen, OT_cost_gen, hist_gen, hist_ld_gen, z_0 = flow.sample(args.train_batch_size)
    z_K, ld_gen, _, hist_gen, hist_ld_gen = NF_obstacle.NF._transform.inverse(z)
    ...
    if args.interaction:
        ...
        F_E, F_P     = compute_F(args, NF_obstacle.obstacle, hist_part, log_prob_0, hist_ld_part, \
...
for step in tbar:
    ...
    # intermediate variables used for vhp computations
    x_list = []
    z_list = []
    params_list = []
    step_list = []
    ...
    # x = next(train_generator).double().to(device) # A batch of trajectories. Shape: B x K x d
    x = next(train_generator).to(device) # A batch of trajectories. Shape: B x K x d
    
    # upper level obj value
    # TODO: see if we need to do a no_grad context here
    l_val = compute_l(NF_obstacle_fun.NF, x)
    l_val_max = l_val
    i_max = 0
    l_list.append(float(l_val))

    # params = tuple(NF_obstacle_fun.parameters())
    # Make params regular Tensors instead of nn.Parameter
    # params = tuple(p.detach().requires_grad_() for p in params)
    # params = tuple([p.detach().cpu().requires_grad_() for p in NF_obstacle_fun.parameters()])
    params_list.append(tuple([p.detach().clone().cpu().requires_grad_() for p in NF_obstacle_fun.parameters()]))

    # lower level updates
    for i in range(args.num_training_steps_inner):
        # beta = args.step_size_inner * args.step_decay_inner/np.sqrt(step+1)
        ...
        loss = compute_mfg_obj(NF_obstacle_fun, x_i, z_i, args)['loss']
        # compute gradient on lower level obj - will be recorded on param.grad
        loss.backward()
        # the above automatically computes grad wrt theta and phi. Don't do the phi update.
        NF_obstacle_fun.obstacle.zero_grad()
        # update each param with its grad.data by taking a simple gradient step
        NF_obstacle_fun = grad_step(NF_obstacle_fun, beta)
        NF_obstacle_fun.NF.zero_grad()
        ...
        # params = tuple(NF_obstacle_fun.parameters())
        # params = tuple(p.detach().requires_grad_() for p in params)
        # params = tuple(p.detach().cpu() for p in params) # seems to be the same as below for our purpose
        # params = tuple(p.detach().cpu().requires_grad_() for p in params)
        # params = tuple([p.detach().cpu().requires_grad_() for p in NF_obstacle_fun.parameters()])
        params_list.append(tuple([p.detach().clone().cpu().requires_grad_() for p in NF_obstacle_fun.parameters()]))
    ...
    # put the parameters at i^* into the model, since that's where the chain rule starts
    # names = [n[0] for n in NF_obstacle_fun.named_parameters()]
    _, names = make_functional(NF_obstacle_fun)
    load_weights(NF_obstacle_fun, names, params_list[i_max], as_params=True, device=x.device)
    # use the functional one with theta_i^* as its param
    g_theta_l = compute_l_grad_i_star(NF_obstacle_fun.NF, x) # B x K*d
    # turn the NN into a pure function
    params, names = make_functional(NF_obstacle_fun)

    def nn_fun(*new_params):
        """Corresponds to F(x; theta, phi). 
        Note that the x_0 are not provided in the inputs to the function,
        so they're assumed to be defined before calling this. This is so that the Autograd only 
        computes the vjp wrt theta.
        """
        # load_weights(flow_obstacle.flow, names[:flow_obstacle.param_len_flow], new_params, as_params=False)
        load_weights(NF_obstacle_fun, names[:NF_obstacle_fun.param_len_flow], new_params, as_params=False, device=x.device)
        # out = NF_obstacle_fun.flow(x_0)
        _, _, _, hist_gen, _ = NF_obstacle_fun.NF._transform.inverse(x[:,0,:])
        hist_gen = partition_hist(hist_gen, args, mode='gen', partition_mode=args.OT_part, LU_last=args.LU_last)
        return hist_gen.view(hist_gen.shape[0], -1) # B x K*d

    def loss_fun(*new_params):
        """Corresponds to L(x, z; theta, phi). 
        Note that the x, z are not provided in the inputs to the function,
        so they're assumed to be defined before calling this. This is so that the Autograd only 
        computes the vhp wrt theta.
        """
        load_weights(NF_obstacle_fun, names, new_params, as_params=False, device=x.device)
        # MFG loss
        loss = compute_mfg_obj(NF_obstacle_fun, x_im1, z_im1, args)['loss']
        return loss

    # do vjp with NN as a pure function
    _, g_theta_i_max = vjp(nn_fun, params[:NF_obstacle_fun.param_len_flow], g_theta_l) # g_theta: same shape as the NF params
    # NOTE: when input is a batch, the vjp sums over the batch dimension. So we divide the gradient by batch size
    g_theta_i_max = tuple([g / args.train_batch_size for g in g_theta_i_max])


    g_zeros_like_phi = tuple([torch.zeros_like(p) for p in params[NF_obstacle_fun.param_len_flow:]])
    # g_phi = copy.deepcopy(g_zeros_like_phi)
    g = g_theta_i_max + g_zeros_like_phi

    # for i in range(i_max, 0, -1): # i goes from i_max to 1
    #     g_theta_i = g[:NF_obstacle_fun.param_len_flow]
    #     # change the input to be consistent with what was used during the lower level updates
    #     x_im1 = x_list[i-1]
    #     z_im1 = z_list[i-1]
    #     # gradient for the obstacle
    #     _, g_theta_i_max_H = vhp(loss_fun, tuple_to_device(params_list[i-1], x.device), g_theta_i_max + g_zeros_like_phi)
    #     # gradient for the NF
    #     _, g_theta_i_H     = vhp(loss_fun, tuple_to_device(params_list[i-1], x.device), g_theta_i + g_zeros_like_phi)
    #     g = vhp_update(g, g_theta_i_H[:NF_obstacle_fun.param_len_flow] + g_theta_i_max_H[NF_obstacle_fun.param_len_flow:],\
    #                  step_size=step_list[i-1])
    ...
    for i in range(i_max, 0, -1): # i goes from i_max to 1
        ...
        x_im1 = x_list[i-1]
        z_im1 = z_list[i-1]
        # gradient for the NF and obstacle
        _, g_theta_i_H = vhp(loss_fun, tuple_to_device(params_list[i-1], x.device), g_theta_i + g_zeros_like_phi)

        # # temp
        # g_1 = [g for g in g_theta_i_H[:NF_obstacle_fun.param_len_flow]]
        # g_2 = [-g for g in g_theta_i_H[NF_obstacle_fun.param_len_flow:]]
        # g = g_1 + g_2

        g = vhp_update(g, g_theta_i_H, step_size=step_list[i-1])
    ...
    # debug
    P = list(NF_obstacle.parameters())
    P_W_1 = P[-4].grad.detach().clone()
    P_W_2 = P[-2].grad.detach().clone()
    P_b_1 = P[-3].grad.detach().clone()
    P_b_2 = P[-1].grad.detach().clone()
    ...
    if (step + 1) % args.monitor_interval == 0:
        ...
        print ("Step: {}, obstacle error: {:.2f}, l: {:.2f}, i_max: {}, lower loss: {}, avg lower loss: {:.2f}, l_list: {}, avg l: {:.2f}".format(\
                step, err, l_val, i_max, lower_loss_list, np.mean(lower_loss_list), l_list, np.mean(l_list)))
        print ("Gradient sum on biases: {:.2f}, {:.2f}".format(P_b_1.sum(), P_b_2.sum()))
        print ("Gradient sum on W's: {:.2f}, {:.2f}".format(P_W_1.sum(), P_W_2.sum()))
        plot_logger.log('steps', step)
        plot_logger.log('lower_mean', float(np.mean(lower_loss_list)))
        plot_logger.log('upper_mean', float(np.mean(l_list)))
