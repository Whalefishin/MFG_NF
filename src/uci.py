...
# os.environ['DATAROOT'] = os.path.join(os.curdir,'data/')
...
def create_transform():
    ...
    # The question is: do we see each composite transform block in the 
    # block as one flow or two flows?
    K = 2 * args.num_flow_steps + 1
...
for step in tbar:
    ...
    # grab data and forward
    batch             = next(train_generator).to(device)
    log_density, _, _ = flow.log_prob(batch)

    if args.lbd_OT == 0:
        loss = - torch.mean(log_density)
    else:
        # sample a batch from the base to compute the OT cost
        _, OT_cost = flow.sample(args.train_batch_size)
        loss       = - torch.mean(log_density) + args.lbd_OT * torch.mean(OT_cost) * K
    loss.backward()
    ...
    if (step + 1) % args.monitor_interval == 0:
        ...
        with torch.no_grad():
            ...
            for val_batch in val_loader:
                log_density_val, _, _ = flow.log_prob(val_batch.to(device).detach())
...
with torch.no_grad():
    ...
    for batch in tqdm(test_loader):
        log_density, _, _ = flow.log_prob(batch.to(device))
