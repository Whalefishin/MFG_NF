...
class CompositeTransform(Transform):
    ...
    def _cascade(inputs, funcs, context):
        ...
        total_ld        = [torch.zeros(batch_size,1).to(inputs.device)]
        ...
        return outputs, total_logabsdet, total_OT_cost, total_outputs, total_ld

    # @staticmethod
    # def _cascade_with_OT(inputs, funcs, context):
    #     batch_size      = inputs.shape[0]
    #     outputs         = inputs
    #     # outputs_prev    = inputs
    #     total_logabsdet = torch.zeros(batch_size)
    #     total_OT_cost   = torch.zeros(batch_size)
    #     # K               = 0

    #     for func in funcs:
    #         outputs, logabsdet, OT_cost = func(outputs, context)
    #         total_logabsdet   += logabsdet
    #         # OT_cost           += torch.norm(outputs - outputs_prev)**2
    #         total_OT_cost     += OT_cost
    ...
    #     return outputs, total_logabsdet, total_OT_cost
    ...
    def _cascade_with_OT(inputs, funcs, context):
        ...
        for func in funcs:
            outputs, logabsdet, OT_cost, outputs_list = func(outputs, context)
            total_logabsdet   += logabsdet
            total_OT_cost     += OT_cost
            total_outputs     += outputs_list
    ...
    #     context = None
    #     funcs   = self._transforms
    #     return self._cascade(inputs, funcs, context)[0]
