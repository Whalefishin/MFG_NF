...
for step in tbar:
    ...
    grad_correctness = gradcheck(LL, params_0)
    print ("Grad check result: ", grad_correctness)
    ...
    if args.debug_opt == 'orig':
        g = grad(l_list[i_max], theta_0 + phi_0, allow_unused=True, retain_graph=True)
        NF_obstacle = set_grad(NF_obstacle, g)
    elif args.debug_opt == 'orig_l_last':
        g = grad(l_list[-1], theta_0 + phi_0, allow_unused=True, retain_graph=True)
        NF_obstacle = set_grad(NF_obstacle, g)
    elif args.debug_opt == 'phi_l_last':
        g = grad(l_list[-1], phi_0, allow_unused=True, retain_graph=True)
        NF_obstacle.obstacle = set_grad(NF_obstacle.obstacle, g)
    ...
    if (step + 1) % args.monitor_interval == 0:
        ...
        if args.verbose_logging:
            if args.debug_opt == 'phi_l_last':
                grad_norm_obs = np.sum([float(torch.norm(p)) for p in g if p is not None])
