...
def parse_arguments():
    ...
    parser.add_argument('--sgd_obs_momentum', type=float, default=0)
...
elif args.optimizer_obs == 'adam':
    optimizer_obs   = optim.Adam(obstacle.parameters(), lr=args.lr_obs, weight_decay=args.l2_reg_obs)
# inner
if args.optimizer_inner == 'sgd':
    optimizer_inner = optim.SGD(NF.parameters(), lr=args.step_size_inner, weight_decay=args.l2_reg_inner, \
            nesterov=args.sgd_inner_nesterov, momentum=args.sgd_inner_momentum)
elif args.optimizer_inner == 'adam':
    optimizer_inner = optim.Adam(NF.parameters(), lr=args.step_size_inner, weight_decay=args.l2_reg_inner)
# NF
...
# debug: in this case, we don't warm start the LL variable 
# if 'IA' not in args.debug_opt:
#     transform_initial, K = create_transform()
#     NF_initial = flows.Flow(transform_initial, P_0).to(device)
#     optimizer_inner = optim_to_use_inner(NF_initial.parameters(), lr=args.step_size_inner, weight_decay=args.l2_reg_inner)
...
for step in range(args.num_training_steps):
    ...
    with higher.innerloop_ctx(NF, optimizer_inner, copy_initial_weights=False) as (NF_inner, NF_inner_opt):
        for i in range(args.num_training_steps_inner):
            ...
            if loss_outer > l_max:
                ...
                l_max = loss_outer
            # logging
            # l_log_list.append(loss_outer.detach())
            # lower_loss_list.append(loss_inner.detach().cpu().numpy())
            l_log_list.append(float(loss_outer))
            lower_loss_list.append(float(loss_inner))
            step_inner += 1

        # compute UL gradient
        if 'PTT' in args.debug_opt:
    ...
    #         NF = set_grad(NF, g_NF)
    # else:
    #     with higher.innerloop_ctx(NF_initial, optimizer_inner, copy_initial_weights=False) as (NF_inner, NF_inner_opt):
    #         for i in range(args.num_training_steps_inner):
    #             # LL objective
    #             x_i = P_1.sample(args.train_batch_size_inner)
    #             z_i = P_0.sample(args.train_batch_size_inner)
...
for i in range(args.num_batch_to_plot):
    if args.tensor_type == 'double':
        x = next(train_generator).to(device).double() # B x K x d
    else:
