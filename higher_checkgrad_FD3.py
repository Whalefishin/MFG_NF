...
def parse_arguments():
    ...
    parser.add_argument('--debug_opt',      type=str, default='IAPTT', choices=
                                                ['IAPTT', 'IA', 'PTT', 'naive'])
...
if args.dataset_name == 'crowd_motion_gaussian_bilevel':
    ...
    args.pretrain_NF_dir = './results/crowd_motion_gaussian/NSF_CL_crowd_motion_2D_Jeff_OT_1e-1_F_2e-1_B=2048_lr=1e-3_N=50k_FD4Simp/crowd_motion_gaussian-best-val-NSF_CL_crowd_motion_2D_Jeff_OT_1e-1_F_2e-1_B=2048_lr=1e-3_N=50k_FD4Simp.t'
...
# NF = NF_iterative(transform, P_0, K).to(device)
# NF = NF_iterative_test(transform, P_0, K).to(device)
# NF = NF_iterative_flatVar(transform, P_0, K).to(device)

# LU_model = transforms.LULinear(features, identity_init=False)
# LU_model = LULinear_iterative(features, identity_init=False)
# LU_model = LULinear_test(features, identity_init=False)
LU_model = PiecewiseRationalQuadraticCouplingTransform(
            mask=utils.create_alternating_binary_mask(features, even=True),
            transform_net_create_fn=lambda in_features, out_features: nn_.ResidualNet(
                in_features=in_features,
                out_features=out_features,
                hidden_features=args.hidden_features,
            ...
            # transform_net_create_fn=lambda in_features, out_features: NN_test(in_features, out_features),
            num_bins=args.num_bins,
            tails='linear',
            tail_bound=args.tail_bound,
            apply_unconditional_transform=args.apply_unconditional_transform
        )
NF = NF_iterative_test([LU_model], P_0, K).to(device)
# NF = NF_iterative_LU(LU_model, P_0, K).to(device)
...
def compute_l_functional(model, theta, names, x):
    # x: B x K x d

    # load params into the network
    load_weights(model, names, theta, as_params=False, device=x.device)
    # load_weights_bootleg(model, theta)
    # compute upper level objective at the specified parameter values
    B = x.shape[0]
    _, _, _, hist_gen, _ = model.inverse(x[:,0,:])
    # hist_gen = partition_hist(hist_gen, args, mode='gen', LU_last=args.LU_last, partition_mode=args.OT_part)
    F = hist_gen.reshape(B,-1) # B x K*d
    x_flat = x.reshape(B, -1)
    l_val = 1/2 * torch.mean(torch.sum((F - x_flat)**2, dim=-1))
...
def compute_l_dummy(NF, x):
    # x: B x K x d
    # compute upper level objective at the specified parameter values
    B_dummy = x.shape[0]
    _, _, _, hist_gen_dummy, _ = NF(x[:,0,:])
    # hist_gen = partition_hist(hist_gen, args, mode='gen', LU_last=args.LU_last, partition_mode=args.OT_part)
    F_dummy = hist_gen_dummy.reshape(B_dummy,-1) # B x K*d
    x_flat_dummy = x.reshape(B_dummy, -1)
    l_val_dummy = 1/2 * torch.mean(torch.sum((F_dummy - x_flat_dummy)**2, dim=-1))

    return l_val_dummy, hist_gen_dummy


def compute_l_functional_dummy(model, theta, names, x):
    # x: B x K x d
...
for step in range(args.num_training_steps):
    ...
    NF_fun  = copy.deepcopy(NF)
    params_0, names = make_functional(NF_fun, device=device)
    ...
    with higher.innerloop_ctx(NF, optimizer_inner, copy_initial_weights=False) as (NF_inner, NF_inner_opt):
        ...
        g_NF_fun  = grad(loss_outer_fun, NF_inner.parameters(time=0), retain_graph=True, allow_unused=True)
        g_obs_fun = grad(loss_outer_fun, obstacle.parameters(), retain_graph=True, allow_unused=True)


    # This is to check if loss_inner and loss_outer are the same for NF and higher-wrapped NF
    NF_copy  = copy.deepcopy(NF)
    optimizer_copy = optim_to_use_inner(NF_copy.parameters(), lr=args.step_size_inner, weight_decay=args.l2_reg_inner)
    ...
    NF_gradcheck_result, NF_grad_diff   = grad_FD_randcheck(NF, obstacle, x, x_list, z_list, loss_outer, \
    ...
    # gradcheck on ALL entries - very slow
    d_list = [10**(-i) for i in range(6,7)]
    diff_list = []
    rel_diff_list = []
    g_obs_FD_list = []
    for d in d_list:
        g_obs_FD = grad_FD(NF, obstacle, x, x_list, z_list, loss_outer, d=d)

        norm_AD = torch.sqrt(sum([torch.sum(g**2) for g in g_obs])) 
        g_diff = [g_AD - g_FD for (g_AD, g_FD) in zip(g_obs, g_obs_FD)]
    ...
    print ("grad from AD: {}".format(g_obs))

    # use FD gradient
    # obstacle = set_grad(obstacle, g_obs)
    obstacle = set_grad(obstacle, g_obs_FD)
    NF = set_grad(NF, g_NF)
    ...
    if (step + 1) % args.monitor_interval == 0:
        ...
        print ("Step: {}, obstacle error: {:.4f}, l LL: {:.4f}, l UL: {:.4f}, i_max: {}, avg lower loss: {:.4f}".format(\
                step, err, loss_outer, loss_UL, i_max, np.mean(lower_loss_list)))
        ...
        log_names = ['steps', 'lower_mean', 'upper_mean', 'B_err']
        log_data  = [step, float(np.mean(lower_loss_list)), float(np.mean(l_log_list)), float(err)]
        plot_logger.log_multiple(log_names, log_data)
        if tbx_logging:
            writer.add_scalar(tag='lower_mean', scalar_value=float(np.mean(lower_loss_list)), global_step=step)
